...
async function channelSearch() {
  ...
  let nextPageToken = '';
  let channels = {};
  let res = await youtube.search.list(createOptions(nextPageToken, '/m/01k8wb'));
  ...
  for (let index = 1; index < totalCycles; index++) {
    ...
    res = await youtube.search.list(createOptions(nextPageToken, '/m/01k8wb'));
    res.data.items.forEach(item => {
      if(!(item.snippet.channelId in channels)){
        channels[item.snippet.channelId] = item.snippet.channelTitle
      }
  ...
  fs.writeFile('knowledge-topic-channels.json', JSON.stringify(channels, null, '\t') , function(err) {
    if(err) {
      console.log(err);
    }
